...
class SimpleNNWrapper:
    ...
    def fit(self, X, y):
        ...
        # Include validation split for tracking val_loss and val_accuracy
        callbacks = [
...
def plot_nn_training_curves(model_name: str, history):
    """Plot side-by-side charts: (1) Loss (train vs val) (2) Accuracy (train vs val)."""
    if history is None:
        return
    try:
        ...
        text_box.insert(tk.END, f"NN curve plotting error: {e}\n")
...
def train_model(model_name, model_instance):
    ...
    try:
        ...
        if isinstance(model_instance, SimpleNNWrapper):
            ...
            # Plot NN curves
            plot_nn_training_curves(model_name, model_instance.history)
        else:
            predictor = Pipeline([
                ("preprocessor", preprocessor),
                ("model", model_instance)
...
def open_prediction_sandbox():
    """Open a window allowing manual feature input and model prediction."""
    if not TRAINED_MODELS:
        text_box.insert(tk.END, "Train at least one model before opening the sandbox.\n")
        text_box.see(tk.END)
        return
    sandbox = tk.Toplevel(root)
    sandbox.title("Prediction Sandbox")
    # Load data once to get categorical choices
    try:
        raw = basic_clean(load_data())
    except Exception as e:
        tk.Label(sandbox, text=f"Data load error: {e}").pack()
        return
    cat_choices = {col: sorted(raw[col].dropna().unique().tolist()) for col in CATEGORICAL_COLUMNS if col in raw.columns}
    entries_num = {}
    vars_cat = {}
    form_frame = tk.Frame(sandbox)
    form_frame.pack(side=tk.TOP, padx=10, pady=10)
    # Numerical inputs
    tk.Label(form_frame, text="Numerical Features", font=('Arial', 10, 'bold')).grid(row=0, column=0, sticky='w', pady=(0,4))
    r = 1
    for num in NUMERICAL_COLUMNS:
        tk.Label(form_frame, text=num).grid(row=r, column=0, sticky='w')
        e = tk.Entry(form_frame, width=12)
        e.grid(row=r, column=1, padx=5, pady=2)
        entries_num[num] = e
        r += 1
    # Categorical inputs
    tk.Label(form_frame, text="Categorical Features", font=('Arial', 10, 'bold')).grid(row=0, column=2, sticky='w', padx=(20,0), pady=(0,4))
    r2 = 1
    for cat in CATEGORICAL_COLUMNS:
        tk.Label(form_frame, text=cat).grid(row=r2, column=2, sticky='w', padx=(20,0))
        var = tk.StringVar()
        opts = cat_choices.get(cat, [''])
        if opts:
            var.set(opts[0])
        om = tk.OptionMenu(form_frame, var, *opts)
        om.config(width=14)
        om.grid(row=r2, column=3, padx=5, pady=2)
        vars_cat[cat] = var
        r2 += 1
    # Model selection
    ...
    tk.Label(form_frame, text="Model").grid(row=max(r,r2)+1, column=0, sticky='w', pady=(10,2))
    tk.OptionMenu(form_frame, model_var, *model_names).grid(row=max(r,r2)+1, column=1, sticky='w', pady=(10,2))
    result_box = tk.Text(sandbox, height=6, width=60)
    result_box.pack(padx=10, pady=10, fill='x')
    def do_predict():
        name = model_var.get()
        model = TRAINED_MODELS.get(name)
        if model is None:
            result_box.delete('1.0', tk.END)
            result_box.insert(tk.END, "Selected model not found.\n")
            return
        # Build row
        row = {}
        # Numericals
        for num, ent in entries_num.items():
            val_txt = ent.get().strip()
            if val_txt == '':
                row[num] = np.nan
            else:
                try:
                    row[num] = float(val_txt)
                except ValueError:
                    row[num] = np.nan
        # Categoricals
        for cat, var in vars_cat.items():
            row[cat] = var.get()
        df_row = pd.DataFrame([row])
        try:
            if hasattr(model, 'predict_proba'):
                proba = model.predict_proba(df_row)[:,1][0]
                pred = int(proba >= CURRENT_THRESHOLD)
            else:
                proba = None
                pred = model.predict(df_row)[0]
            result_box.delete('1.0', tk.END)
            result_box.insert(tk.END, f"Model: {name}\n")
            if proba is not None:
                result_box.insert(tk.END, f"Probability (Churn=1): {proba:.4f}\n")
                result_box.insert(tk.END, f"Threshold: {CURRENT_THRESHOLD:.2f}\n")
            result_box.insert(tk.END, f"Predicted Class: {pred}\n")
        except Exception as e:
            result_box.delete('1.0', tk.END)
            result_box.insert(tk.END, f"Prediction error: {e}\n")
    tk.Button(sandbox, text="Predict", command=do_predict).pack(pady=(0,10))
...
if __name__ == "__main__":
    ...
    # Prediction sandbox button
    sandbox_btn = tk.Button(button_frame, text="Prediction Sandbox", command=open_prediction_sandbox)
    sandbox_btn.pack(fill='x', pady=3)
