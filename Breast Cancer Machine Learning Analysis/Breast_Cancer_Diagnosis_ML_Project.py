...
def main():
    ...
    # Plot top 9 correlated features
    top9_cols = correlations.index[:9]
    plt.figure(figsize=(18, 12))
    for i, col in enumerate(top9_cols, 1):
        plt.subplot(3, 3, i)
        plt.scatter(df[col], df['diagnosis_encoded'], alpha=0.5)
        plt.xlabel(col)
        plt.ylabel('diagnosis_encoded')
        plt.title(f'{col} vs diagnosis_encoded')
    plt.tight_layout()
    plt.show()
